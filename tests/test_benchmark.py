...
def test_JDA_MNA(NWN_benchmark_JDA: NanowireNetwork) -> None:
    # Get benchmark network
    NWN = NWN_benchmark_JDA
    assert NWN.graph["type"] == "JDA"
    ...
    # Compare G matrix
    mnns_G = mnns.create_matrix(
        NWN, "conductance", [(0,)], [], False
    ).toarray()
    assert np.sum(np.abs(G - mnns_G)) < 1e-12
    ...
    # Compare G matrix with grounded node
    mnns_G = mnns.create_matrix(
        NWN, "conductance", [(0,)], [(1,)], False
    ).toarray()
    assert np.sum(np.abs(G - mnns_G)) < 1e-12
...
def test_MNR_resistance(NWN_benchmark_MNR: NanowireNetwork) -> None:
    ...
    R_MNR = (
        20 + Rin1 + 20 + Rin2 + 1 / (1 / (Rin3 + 20) + 1 / (20 + Rin4 + 20))
    )  # ~74.618
